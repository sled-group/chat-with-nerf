...
def get_mesh_object(
    ...
    if geometry.has_vertex_colors():
        ...
        color_strings = ["rgb({}, {}, {})".format(r, g, b) for r, g, b in colors_255]
        mesh_3d = go.Mesh3d(
            x=vertices[:, 0],
            y=vertices[:, 1],
            z=vertices[:, 2],
            ...
            j=triangles[:, 1],
            k=triangles[:, 2],
            vertexcolor=color_strings,  # Using vertex colors directly
            opacity=1,
            # we can set light parameters for better visualization
        )
    else:
...
def get_lineset_object(geometry, color=(0, 0, 1)):
    x = []
    y = []
    z = []
    ...
    line_3d = go.Scatter3d(
        x=x, y=y, z=z, mode="lines", line=dict(width=10, color=color)
    )
    return line_3d


def get_graph_objects(
    geometry_list, mesh_show_wireframe=False, point_sample_factor=1, color=None
):
    graph_objects = []
    for geometry in geometry_list:
        ...
        if geometry_type == o3d.geometry.Geometry.Type.LineSet:
            graph_objects.append(get_lineset_object(geometry, color))
...
def get_plotly_fig(
    ...
    graph_objects = get_graph_objects(
        geometry_list, mesh_show_wireframe, point_sample_factor, color
    )
    new_list = []
